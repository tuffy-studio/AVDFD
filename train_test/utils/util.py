...
def plot_precision_recall_curve(y_true, y_scores, class_name):
    """
...
def plot_roc_curve(y_true, y_scores, class_name):
    ...
    plt.grid()
    #plt.show()
    plt.savefig(f"ROC_curve_{class_name}.png")


def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, class_names=None, normalize=False):
    """
    绘制混淆矩阵（支持归一化）
    ...
    plt.title(titlename)
    #plt.show()
    filename = f"CM{'_normalized' if normalize else ''}.png"
